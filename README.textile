...
Consider a simple blog app, where we need to alert subscribers everytime a new post is published. If the number of subscribers is too high, the hosting may not allow so many emails to be sent.
...
Now that the queue of awaiting emails has been filled, we can configure the CronMailer shell. Again, the available settings are very close to the core EmailComponent, with only one new parameter, 'limit', which is the number of emails to be sent on each execution of the shell. Check you hosting offer to configure the limit accordingly.
